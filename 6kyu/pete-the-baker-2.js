...
function getMissingIngredients(recipe, added) {
    ...
    let numberOfCakes = Math.max(
        ...ingredients.map(
            ingredient => Math.ceil(added[ingredient] / recipe[ingredient]) || 0
        )
    )

    let missingIngredients = ingredients.reduce((accu, curr) => {
        let needed = recipe[curr] * numberOfCakes - (added[curr] || 0)
        return needed ? { ...accu, [curr]: needed } : { ...accu }
    }, {})
